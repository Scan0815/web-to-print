...
| --------------- | ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------- | ----------------------------------------------------------------- |
| `activeViewId`  | `active-view-id`  | Id of the decoration currently being edited. Two-way: assign a valid id to switch, and read it back (as a DOM **property**, not the attribute) to learn the shown view — the editor writes the resolved view onto it once real `views` exist, including the initial one. Defaults to the `isDefault` view, else the first. An id not in `views` is ignored by the watcher (the imperative `setActiveView` throws instead) and corrected to the default on the next `views` change — so after assigning an unknown id the property is momentarily stale until then. | `string`        | `undefined`                                                       |
...
| `showViewStrip` | `show-view-strip` | Show the built-in decoration strip. Turn off to build your own switcher around `activeViewId`. Defaults to true on purpose: the strip is the editor's out-of-the-box way to reach the other decorations, so it must be present unless a host opts out.                                                                                                                                                                                                                                                                                                             | `boolean`       | `true`                                                            |
