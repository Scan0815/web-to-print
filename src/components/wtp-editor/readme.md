...
| --------------- | ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- | ----------------------------------------------------------------- |
| `activeViewId`  | `active-view-id`  | Id of the decoration currently being edited. Two-way: set it to switch, read it (as a DOM **property**, not the attribute) to learn what is shown — the editor writes the resolved view back once real `views` exist, including the initial one. Defaults to the `isDefault` view, else the first. An id that is not in `views` is replaced with that default rather than kept.                                 | `string`        | `undefined`                                                       |
